 ...
 *
 * ユーザーが Config シートで入力するのは以下だけ:
 * - 対象フォルダの Google Drive URL / ID
 * - 対象ユーザーのメールアドレス
 *
 * 表示名、Folder ID、Actor ID、状態は自動入力する。
 */
...
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu('Re:年モノ')
    .addItem('初期セットアップ', 'setupReinenMonoWorkbook')
    .addItem('Configを更新', 'refreshReinenConfig')
    .addSeparator()
    .addItem('Dataを更新', 'runReinenMono')
    .addItem('週次通知を今すぐ実行', 'runWeeklyReinenDigest')
    .addItem('週次トリガーを作り直す', 'setupWeeklyReinenTrigger')
    .addSeparator()
...
function setupReinenMonoWorkbook() {
  ...
  const legacy = captureExistingConfigInputs_(config);
  initializeConfigSheet_(config, legacy);
  ...
  ss.toast('初期セットアップが完了しました。黄色のセルだけ入力してください。', 'Re:年モノ', 6);
  return ss.getUrl();
}
...
function captureExistingConfigInputs_(sheet) {
  ...
  if (marker === 'Re:年モノ 設定' && newHeader.indexOf('フォルダURL') >= 0) {
    result.folders = sheet.getRange(REINEN_SHEET_CONFIG.FOLDER_FIRST_ROW, 1, 20, 1)
      .getDisplayValues().flat().map((v) => String(v || '').trim()).filter(Boolean);
    result.users = sheet.getRange(REINEN_SHEET_CONFIG.USER_FIRST_ROW, 1, 20, 1)
...
function initializeConfigSheet_(sheet, preserved) {
  ...
  sheet.getRange('A1').setValue('Re:年モノ 設定').setFontSize(16).setFontWeight('bold');
  sheet.getRange('A2:D2').merge();
  sheet.getRange('A2')
    .setValue('黄色のセルだけ入力してください。グレーの列は自動入力されます。空欄の行は対象外です。')
  ...
  sheet.getRange(REINEN_SHEET_CONFIG.FOLDER_FIRST_ROW, 2, folderRows, 3)
    .setBackground('#f3f3f3').setFontColor('#5f6368');
  ...
  sheet.getRange('A52:D53').merge();
  sheet.getRange('A52')
    .setValue('入力後は「Re:年モノ > Configを更新」を実行してください。フォルダ名・ID、ユーザー名・Actor IDは自動取得されます。')
    .setWrap(true).setFontColor('#5f6368');
  ...
  sheet.getRange('A1:D53').setVerticalAlignment('middle');
}
...
function readSelectedUsers_(sheet) {
  return sheet.getRange(REINEN_SHEET_CONFIG.USER_FIRST_ROW, 1, 20, 4).getDisplayValues()
    ...
    .map((row) => {
      ...
      if (!/^people\//.test(actorId)) throw new Error(`${email}: Actor IDを解決できません。Configを更新してください。`);
...
function getReinenSpreadsheet_() {
  ...
  if (!id) throw new Error('SPREADSHEET_ID が未設定です。初期セットアップを実行してください。');
  return SpreadsheetApp.openById(id);
}

function requireActiveBoundSpreadsheet_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  if (!ss) throw new Error('このスクリプトはコンテナバインドスクリプトとして使用してください。');
  return ss;
}

function requireConfigSheet_(ss) {
  const sheet = ss.getSheetByName(REINEN_SHEET_CONFIG.CONFIG_SHEET);
  if (!sheet) throw new Error('Configシートがありません。初期セットアップを実行してください。');
  return sheet;
}
