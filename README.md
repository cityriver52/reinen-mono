...
- 開始目安
- 重要度 — 現在の候補内のscoreを `●●●○○` のような5段階で相対表示
- ファイル名（Driveへのリンク）
- Driveルートからのフォルダ階層
- `昨年8月10日～8月25日の間に6回編集` のような活動要約
- `通知対象 / あとで / 今年は不要` の通知状態

表示順は **開始目安が近い順**。開始目安が同じ候補はscoreが高いものを先に表示します。

重要度は絶対評価ではなく、その時点のView内で最高scoreの候補を `●●●●●` とした相対表示です。scoreの細かな数値差を前面に出さず、「どれが特に重要そうか」だけを視覚的に把握できるようにします。
