...
現在の主眼は、**昨年度の中でも今ごろに活動が偏っていたファイル**を見つけ、静かなPushで提示すること。
...
比較対象は、その1年前の日付が属する **年度（4月1日〜翌年3月31日）**。

```text
seasonal_rate = 季節ウィンドウ活動日数 / 季節ウィンドウ日数
background_rate = 年度内その他期間の活動日数 / 年度内その他期間の日数
seasonal_lift = seasonal_rate / background_rate
```

他時期が0日のときは無限大にせず、年度内で1日活動した相当の下限値を使う。

デフォルトでは `seasonal_lift >= 2.0` を要求する。

これにより、年度を通して継続的に使うファイルを除外しやすくする。

**直近90日や今年の稼働有無は条件に含めない。**

4月初旬・3月末付近では、季節ウィンドウを比較対象年度の中にクリップする。
...
候補に絞り込んだ後、Drive API v3で親フォルダをルート方向へ辿り、`folder_path` として保存する。
